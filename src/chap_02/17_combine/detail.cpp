...
Detail::Detail(QWidget *parent, Qt::WindowFlags f1)
	: QWidget(parent,f1)
{
    label1 = new QLabel(tr("Country/Zone:")); //  国家地区
    label2 = new QLabel(tr("Province:"));          // 省份
    label3 = new QLabel(tr("City:"));                   // 城市
    label4 = new QLabel(tr("Discription:"));      // 说明
    
    comboBoxCountry = new QComboBox();
    comboBoxCountry->insertItem(0,tr("China"));  // 中华人民共和国
    
    comboBoxProvince = new QComboBox();
    comboBoxProvince->insertItem(0,tr("JiangSu"));  // 江苏省
