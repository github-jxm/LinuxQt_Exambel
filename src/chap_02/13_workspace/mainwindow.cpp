...
{
    QMenu *layoutMenu = menuBar()->addMenu(tr("Layout")); // 布局
    
    QAction *arrange = new QAction(tr("Arrange Icons"),this);   // 排列图标
    connect(arrange,SIGNAL(triggered()),workSpace,SLOT(arrangeIcons()));
    layoutMenu->addAction(arrange);
    
    QAction *tile = new QAction(tr("tile"),this);  // 平铺
    connect(tile,SIGNAL(triggered()),workSpace,SLOT(tile()));
    layoutMenu->addAction(tile);
    
    QAction *cascade = new QAction(tr("cascade"),this);  // 层叠
    connect(cascade,SIGNAL(triggered()),workSpace,SLOT(cascade()));
    layoutMenu->addAction(cascade);
    
    
    QMenu *otherMenu = menuBar()->addMenu(tr("Other"));  // 其他
    
    QAction *scrollAct = new QAction(tr("Scroll"),this);   // 滑动条
    ...
    QAction *nextAct = new QAction(tr("Next"),this);  // 下一个窗口
    connect(nextAct,SIGNAL(triggered()),workSpace,SLOT(activateNextWindow()));
    otherMenu->addAction(nextAct);
    
    QAction *previousAct = new QAction(tr("Previous"),this);  //  前一个窗口
